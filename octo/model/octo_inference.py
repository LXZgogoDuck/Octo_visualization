...
class OctoInference:
    ...
    def step(self, image: np.ndarray, task_description: Optional[str] = None, goal_image: Optional[np.ndarray] = None, *args, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        """
        Input:
        ...
        if (
                ...
                or (goal_image is not None and not np.array_equal(goal_image, getattr(self, "goal_image", None)))
        ):
            self.reset(task_description=task_description, goal_image=goal_image)
        ...
        input_observation = {"image_primary": images, "pad_mask": pad_mask, "timestep_pad_mask": pad_mask}
        ...
        return raw_action, action, attention_weights
